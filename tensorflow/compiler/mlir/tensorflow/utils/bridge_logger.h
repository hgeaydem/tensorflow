...
class BridgeLoggerConfig : public mlir::PassManager::IRPrinterConfig {
 ...
 private:
  ...
  static bool MatchesFilter(const std::string& str,
                            ...
                            bool exact_match);
  // Checks the environment variable to determine whether to enable logging only
  // for top-level passes. Returns true if the environment variable is set
  // to "1", "true", or "yes" (case-insensitive).
  static bool GetEnableOnlyTopLevelPasses();
  ...
  // If true, only log pass invocations where the operation is a top-level
  // operation. This is to reduce the amount of dumps generated by nested
  // operations. Default is set to false.
  const bool enable_only_top_level_passes_;
};
