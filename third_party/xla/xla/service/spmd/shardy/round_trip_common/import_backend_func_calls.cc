...
class BackendFuncCallPattern : public OpConversionPattern<CallOp> {
 public:
  explicit BackendFuncCallPattern(MLIRContext* context, SymbolTable symbolTable)
  ...
  mlir::LogicalResult matchAndRewrite(
    ...
    auto discardableAttrs = callOp->getDiscardableAttrs();
    mlir::SmallVector<mlir::NamedAttribute> namedCompAttrs;
    llvm::copy_if(discardableAttrs, std::back_inserter(namedCompAttrs),
    ...
    auto namedCompOp = rewriter.replaceOpWithNewOp<NamedComputationOp>(
        ...
        /*outShardings=*/
        callOp->getAttrOfType<mlir::sdy::TensorShardingPerValueAttr>(
            kShardingAttr));
 ...
 public:
  ...
  void runOnOperation() final {
    ...
    SymbolTable& symbolTable =
        symbolTableCollection.getSymbolTable(getOperation());
