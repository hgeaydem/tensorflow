...
class AllocationValue {
 public:
  ...
  struct Use {
    ...
    // A sync data movement operand of this use. TODO(mehrdadk): extend this to
    // support multiple sync data movement operands.
    HloInstruction* sync_data_movement_operand = nullptr;
...
// Represents an entry in the work list for allocating an AllocationValue.
struct AllocAllocationValuesWorkListEntry {
  // The index of the use that we are processing in the uses vector.
  int use_idx;
  // The uses of the AllocationValue that we are processing.
  const std::vector<AllocationValue::Use>* uses;
  // The index of the allocation value that we are going to enqueue the
  // resulting allocation to its allocation sequence.
...
class MsaAlgorithm : public GlobalDecreasingSizeBestFitHeap<HloValue> {
 public:
  ...
  // those values that are produced or consumed by a synchronous data movement
  // instruction.
 ...
 private:
  ...
  struct AllocationRequest {
    ...
    // If the resulting allocation doesn't represent the allocation_value (e.g.,
    // when converting a synchronous slice to asynchronous), enqueue it to a
    // separate allocation sequence, distinct from the one associated with the
    // original allocation_value.
    AllocationValue* queuing_allocation_value;
    // If a use is not allocated because it's going to be replaced with an async
    // use, we need to make sure the operand lifespan is extended until the end
    // of the async use.
    bool is_extension_only;
  ...
  enum class AsyncConversionFailureReason {
    kSuccess = 0,
  ...
  AsyncConversionFailureReason IsReplaceableSyncSliceCandidate(
  ...
  bool IsInstructionPendingReplacements(const AllocationValue::Use& use) const {
    return IsInstructionPendingReplacements(use.hlo_use.instruction);
  }
  ...
  // asynchronous copies.
  absl::flat_hash_map<const HloInstruction*, AsyncConversionFailureReason>
